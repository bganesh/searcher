<script>
  ...
  async function performSearch(resetPage = false) {
    ...
    try {
      const response = await fetch('http://localhost:9200/text_files/_search', {
        method: 'POST',
        headers: {
  ...
  // Function for main search button
  function handleSearch() {
    performSearch(true);  // true means reset page to 1
  }
  ...
  function changePage(newPage) {
    if (newPage >= 1 && newPage <= totalPages) {
      currentPage = newPage;
      performSearch(false);  // false means keep current page
    }
  }
    ...
    <div class="results">
      {#each results as result}
        <div class="result-card">
          <!-- <div class="result-title"><a href="file://{result._source.filepath}" target="_blank" rel="noopener noreferrer"> -->
          <div class="result-title"><a href="http://localhost:3144/{result._source.filepath}" target="_blank" rel="noopener noreferrer">
            {result._source.filename} </a> </div>
